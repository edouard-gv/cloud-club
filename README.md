...
```bash
cloud-club$ aws s3 mb s3://BUCKET_NAME
```

To prepare the application for deployment, use the `sam package` command.

```bash
cloud-club$ sam package \
...
```bash
cloud-club$ sam deploy \
    --template-file packaged.yaml \
    --stack-name cloud-club \
...
```bash
cloud-club$ aws cloudformation describe-stacks \
    --stack-name cloud-club \
...
```bash
cloud-club$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
cloud-club$ sam local start-api
cloud-club$ curl http://localhost:3000/
```
...
```bash
cloud-club$ sam logs -n HelloWorldFunction --stack-name cloud-club --tail
```
...
```bash
cloud-club$ pip install pytest pytest-mock --user
cloud-club$ python -m pytest tests/ -v
```
...
```bash
cloud-club$ aws cloudformation delete-stack --stack-name cloud-club
cloud-club$ aws s3 rb s3://BUCKET_NAME
```
